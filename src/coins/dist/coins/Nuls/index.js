...
const nuls_sdk_js_1 = __importDefault(require("nuls-sdk-js"));
...
const generateAddress = async () => {
    const { address, pri: privateKey } = nuls_sdk_js_1.default.newAddress(chainId, '', '');
...
const importPrivateKey = async (privateKey) => {
    const importByKey = nuls_sdk_js_1.default.importByKey(chainId, privateKey, '', '');
...
const validateAddress = (address) => {
    try {
        const verify = nuls_sdk_js_1.default.verifyAddress(address);
...
const Plus = (nu, arg) => {
    let newPlus = new bignumber_js_1.default(nu);
...
const createTx = async (props) => {
    ...
    if (getPubKey) {
        const params = await (0, api_1.getTxParams)('nuls', addressFrom);
        const { inputs, outputs } = getInputsOutputs({
            fromAddress: addressFrom,
            toAddress: addressTo,
            amount: (0, exports.formatValue)(amount, 'to'),
        }, params);
        const assembleTx = nuls_sdk_js_1.default.transactionAssemble(inputs, outputs, '', 2);
        return nuls_sdk_js_1.default.transactionSerialize(privateKey, getPubKey, assembleTx);
    }
    return null;
};
